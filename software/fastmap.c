	...
	while (sw_handshake[nthreads] != 4) {
		for (i = 0; i < nthreads; ++i) {
    		...
    		*handshake = 0;
				...
				// The worker has decisded to do on CPU
				if (sw_handshake[i] == 0)
					continue;
				...
				*worker_mem[i] = 0;
				...
				for (j = 0; j < read_num; ++j) {
					...
					//fprintf(stderr,"0-%ul,1-%ul,2-%ul,3-%ul,4-%ul",*(SPL_BWT_output_ptr),*(SPL_BWT_output_ptr+1),*(SPL_BWT_output_ptr+2),*(SPL_BWT_output_ptr+3),*(SPL_BWT_output_ptr+4));
					//fprintf(stderr,"read_num=%d,out_num=%d\n",*(SPL_BWT_output_ptr),out_num);
					bwtint_t out_size = 0;
					if (out_num % 2)
...
//					usleep(1);
	...
	fprintf(stderr, "Shutdown HARP management, total kernel time %fs\n", afu_time);
	pthread_exit(0);
}
